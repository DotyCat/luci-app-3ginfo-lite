...
#
# (c) 2021-2025 modified by Rafa? Wabik - IceG - From eko.one.pl forum
#
...
ucs2_hex_to_utf8() {
	...
	decoded=$(printf "%b" "$bytes" | iconv -f UTF-16BE -t UTF-8 2>/dev/null)
	[ -n "$decoded" ] || return 1
...
O=""
if [ -e /usr/bin/mipc_wan_cli ]; then
    O=$(mipc_wan_cli --at_cmd "AT+CPIN?;+CSQ;+COPS=3,0;+COPS?;+COPS=3,2;+COPS?;+CREG=2;+CREG?")
elif [ -e /usr/bin/sms_tool ]; then
...
if [ -n "$TCOPS" ]; then
	DECODED_TCOPS=$(ucs2_hex_to_utf8 "$TCOPS")
	[ -n "$DECODED_TCOPS" ] && TCOPS="$DECODED_TCOPS"
fi
...
if [ -e /usr/bin/mipc_wan_cli ]; then
    isp=$(mipc_wan_cli --at_cmd "AT+COPS?" | sed -n '1p' | cut -d '"' -f2 | tr -d '\r')
else
...
if [ -e /usr/bin/mipc_wan_cli ]; then
    manufacturers_name=$(mipc_wan_cli --at_cmd "AT+CGMI" | sed -n '1p' | cut -d '"' -f2 | tr -d '\r'| tr 'A-Z' 'a-z' | cut -d ' ' -f1)    
    device_name=$(mipc_wan_cli --at_cmd "AT+CGMM" | sed -n '1p' | cut -d '"' -f2 | cut -d '-' -f1 | tr -d '\r' | tr 'A-Z' 'a-z')
    REGOK=0
    [ "x$REG" == "x1" ] || [ "x$REG" == "x5" ] || [ "x$REG" == "x6" ] || [ "x$REG" == "x7" ] && REGOK=1
    if [ -e "$RES/modem/mtk/$manufacturers_name/$device_name" ]; then
        . "$RES/modem/mtk/$manufacturers_name/$device_name"
    fi
elif [ -e /usr/bin/sms_tool ]; then
	REGOK=0
	[ "x$REG" == "x1" ] || [ "x$REG" == "x5" ] || [ "x$REG" == "x6" ] || [ "x$REG" == "x7" ] && REGOK=1
...
"roaming":"$(sanitize_string "$ROAMING")",
...
"mac_rate":"$(sanitize_string "$MAC_RATE")",
"rssi":"$(sanitize_number "$RSSI")",
"sinr":"$(sanitize_number "$SINR")",
"ambr":"$(sanitize_string "$AMBR")",
"prssi":"$(sanitize_detail "$PRSSI")",
"prsrp":"$(sanitize_detail "$PRSRP")",
"psinr":"$(sanitize_detail "$PSINR")",
"pbw":"$(sanitize_detail "$PBW")",
"pspeed":"$(sanitize_detail "$PSPEED")",
"pmcs":"$(sanitize_detail "$PMCS")",
"ptwr":"$(sanitize_detail "$PTWR")",
"s1rssi":"$(sanitize_detail "$S1RSSI")",
"s1rsrp":"$(sanitize_detail "$S1RSRP")",
"s1sinr":"$(sanitize_detail "$S1SINR")",
"s1bw":"$(sanitize_detail "$S1BW")",
"s1speed":"$(sanitize_detail "$S1SPEED")",
"s1mcs":"$(sanitize_detail "$S1MCS")",
"s1twr":"$(sanitize_detail "$S1TWR")",
"s2rssi":"$(sanitize_detail "$S2RSSI")",
"s2rsrp":"$(sanitize_detail "$S2RSRP")",
"s2sinr":"$(sanitize_detail "$S2SINR")",
"s2bw":"$(sanitize_detail "$S2BW")",
"s2speed":"$(sanitize_detail "$S2SPEED")",
"s2mcs":"$(sanitize_detail "$S2MCS")",
"s2twr":"$(sanitize_detail "$S2TWR")",
"s3rssi":"$(sanitize_detail "$S3RSSI")",
"s3rsrp":"$(sanitize_detail "$S3RSRP")",
"s3sinr":"$(sanitize_detail "$S3SINR")",
"s3bw":"$(sanitize_detail "$S3BW")",
"s3speed":"$(sanitize_detail "$S3SPEED")",
"s3mcs":"$(sanitize_detail "$S3MCS")",
"s3twr":"$(sanitize_detail "$S3TWR")",
"s4rssi":"$(sanitize_detail "$S4RSSI")",
"s4rsrp":"$(sanitize_detail "$S4RSRP")",
"s4sinr":"$(sanitize_detail "$S4SINR")",
"s4bw":"$(sanitize_detail "$S4BW")",
"s4speed":"$(sanitize_detail "$S4SPEED")",
"s4mcs":"$(sanitize_detail "$S4MCS")",
"s4twr":"$(sanitize_detail "$S4TWR")"
}
EOF
exit 0
